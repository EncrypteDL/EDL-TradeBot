...
// try tries to place a but order.
...
func tryBuy(botCfg *BotCfg, state *State, client *binance.Client, price float32) {
	pbChange := percentChange(state.Data.LastSellPrice, price)

	if botCfg.StopEntryPrice != 0 && !shouldBuyStopEntry(botCfg, pbChange) {
		log.Debug().Msgf(
			"Not buying, percent change between price and stop entry price@%.5f",
			pbChange,
		)
		return
	} else if !shouldBuy(botCfg, pbChange) {
		log.Debug().Msgf(
